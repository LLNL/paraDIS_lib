...
def usage():
    print "Usage:  jaime2pov.py infile outfile [logfile]"
...
def WriteTriangle(nodes, tricolor):
    print "WriteTriangle(%s, %s)"%(str(nodes), str(tricolor))
    nodestrings = []
    for node in nodes:
...
def WriteFan(fan):
    ...
    discolor=fan[0]
    tricolor=fan[1]
    print "tricolor is", tricolor
    nodelist=CreateActualNodeList(fan[2])
    if (fan[3]):
        nodelist = [nodelist[0]] + InterpolateNodes(nodelist[1:])
        
    print "WriteFan: fully interpolated nodelist is", str(nodelist)
...
try: 
    for realline in infile.xreadlines():
        ...
        if not line or  line[0] == 'VARIABLES="X"' or line[0][0] == "#":
            ...
            continue
        elif line[0] == "DISLOC=" or line[0] == "INTERPOLATE_DISLOC=" or line[0] == "DISLOCATION=" or line[0] == "INTERPOLATE_DISLOCATION=":
