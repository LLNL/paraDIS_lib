#!/usr/local/bin/bash
# $Id: makewormmovie.sh,v 1.8 2006/02/27 16:40:50 rcook Exp $
...
padlength=0
nopadding=false
datadir='/nfs/tmp0/rcook/paradis/2005-11/movie/pov'
verbose=0
# frame and file numbers are one-based
povfirst=1
povlast=399
...
function usage () {
    ...
    echo "  -movidir dir (images): put images in named directory (create if nonexistent)" 
...
if [ $prologframes -gt 0 ] ; then
    ...
    prologstep=`$python -c "print ($animstart)/$prologframes"`
fi
...
if [ $animframes -gt 0 ] ; then
	...
	animstep=`$python -c "print ($animend-$animstart)/($animframes-1)"`
fi
    else 
	animstep=0
    fi
...
if [ $epilogframes -gt 0 ] ; then
    ...
    epilogstep=`$python -c "print (1.0-$animend)/$epilogframes"`
    epilogstart=`$python -c "print $animend+$epilogstep"`
fi
...
if [ x$spline != x ] ; then 
	...
	currentspline=`$python -c "print (${firstframe}-1) '*' $prologstep"`     
	...
	currentspline=`$python -c "print $animstart + ($firstframe - 1 - $prologframes) * $animstep"`    
    else
	currentpov=$povlast
	currentspline=`$python -c "print $animend + ($firstframe - $prologframes - $animframes) * $epilogstep"`
    fi
fi
...
while [ $currentframe -le $lastframe ] ; do
	...
	if [ x$spline = x ] ; then 
	    currentspline=`$python -c "print (${currentframe}-1) * $prologstep"` 
	fi
    else
	...
	if [ x$spline = x ] && [ $currentframe -le `expr $prologframes + $animframes` ] ; then
	    currentspline=`$python -c "print $animstart + ($currentframe - 1 - $prologframes) * $animstep"`
	else
	    currentspline=`$python -c "print $animend + ($currentframe - $prologframes - $animframes) * $epilogstep"`
	fi
    fi
    ...
    #tilestart=`$python -c "print ${tilenum}.0/${numtiles}.0"`
    #tileend=`$python -c "print (${tilenum}.0+1.0)/${numtiles}.0"`
    ...
    cp ${datadir}/$incfile $combined_pov_file
