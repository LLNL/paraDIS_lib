...
for name in ['numarms', 'numcurves', 'numnodes', 'numsegs']:
	exec('print("%ss: %%d  in %f seconds, %ss/sec = %%f " %% (%s, %s/%f))' %
		 (name, elapsed, name, name, name, elapsed))		   

numentries = len(data['timing']['elapsed'])
for thing in ['arms', 'segments', 'nodes', 'curves']:
	print "%15s%15s%15s"%("elapsed", thing, "%s/sec"%thing)
	for n in range(numentries):
		exec("elapsed = %f"%data['timing']['elapsed'][n])			
		exec("numthings = data['timing']['%s'][%d]"%(thing, n))
		if elapsed == 0:
			thingspersec = 0
		else:
			thingspersec = float(numthings)/elapsed
		exec("print('%%15.2f%%15.2f%%15.2f' %% (%f, %f, %f))"%(elapsed, numthings, thingspersec))
