...
try: 
    for line in infile:
        ...
        atom ={"ID":atomID,"atomType":atomType, "radius":float(line[0]), "raw location": fileLoc, "location":loc, "color":float(line[7])}
        gAtoms.append(atom)
...
for atom in gAtoms:
    ...
    outfile.write("make_atom(%f, %f, <%f,%f,%f>, \"%s\")\n"%(atom["radius"], atom["color"], atom["location"][0], atom["location"][1], atom["location"][2], atom['atomType']))
