...
#===================================================
def WriteDislocationSegment(node1,node2, discolor):
    outfile.write("// segment connecting nodes %s and %s with color %d\n"%(node1, node2, discolor))
    nodestrings = []
    for node in [node1,node2]:
        nodestrings.append("<%f,%f,%f>"%(gNodes[node][0], gNodes[node][1], gNodes[node][2]))

    outfile.write("make_dislocation(%s, %s, %d)\n"%
                  (nodestrings[0], nodestrings[1], discolor))
...
def WriteFan(fan):
    ...
    while nodenum < len(nodelist):
        WriteDislocationSegment(nodelist[nodenum], nodelist[nodenum-1], discolor)
        if nodenum+1 < len(fan):
            WriteTriangle([nodelist[0],nodelist[nodenum],nodelist[nodenum+1]], tricolor)
        nodenum = nodenum+1
    WriteDislocationSegment(nodelist[0], nodelist[len(fan)-1], discolor)
    return
...
def WriteDislocation(disloc):
    ...
    while segnum < len(nodelist):
        WriteDislocationSegment(nodelist[segnum], nodelist[segnum-1], discolor)
        segnum=segnum+1
    return
...
def WriteStrip(strip):
    ...
    for striphalf in striphalves:
        ...
        while (nodenum < len(striphalf)):
            WriteDislocationSegment(striphalf[nodenum-1], striphalf[nodenum], discolor)
            nodenum = nodenum + 1
